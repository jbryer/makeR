...
\begin{itemize}
	...
	\item \code{Properties} - a list of the project properties. See also \code{getProperties()}.
	...
	\item \code{file.info} - the file info from the last time the \file{PROJECT.xml} file was read.
\end{itemize}
...
builder.rnw <- function(file, theenv, ...) \{
  ...
  thecall = paste('Rscript -e "', envstr,
      ' Sweave(\'', file, '\')"', sep='')
  system(thecall)
  texi2pdf(paste(substr(file, 1,
      (nchar(file)-4)), '.tex', sep=''))
  return(paste(substr(file, 1,
      (nchar(file)-4)), '.pdf', sep=''))
\}
\end{example}
